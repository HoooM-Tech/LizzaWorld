...
export function FeaturedGrid({ images }: FeaturedGridProps) {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-7xl mx-auto">
      ...
      <motion.figure
        className="group relative overflow-hidden bg-charcoal/5 row-span-2"
        initial={{ opacity: 0, x: -20 }}
        whileInView={{ opacity: 1, x: 0 }}
        viewport={{ once: true, margin: "-180px" }}
        transition={{ duration: 0.6 }}
      >
        <Link href={INSTAGRAM_URL} target="_blank" rel="noopener noreferrer" className="block h-full">
          <Image
            src={visualSources[0] ?? fallbackImages[0]}
            alt={captions[0]}
            fill
            unoptimized
            className="object-cover transition-all duration-700 group-hover:scale-110 group-hover:grayscale rounded-sm"
          />
      ...
      >
        <Link href={INSTAGRAM_URL} target="_blank" rel="noopener noreferrer" className="block">
          <div className={imageConfig[1].aspect}>
            <Image
      ...
      >
        <Link href={INSTAGRAM_URL} target="_blank" rel="noopener noreferrer" className="block">
          <div className={imageConfig[2].aspect}>
            <Image
